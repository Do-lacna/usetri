...
const TotalSavedCard: React.FC<ProductCardProps> = () => {
  ...
  return (
    <Card className="w-full px-4 py-2 bg-card border border-border">
      <Text className="text-lg font-semibold text-card-foreground">Vaše úspory</Text>
      <View className="flex-row space-x-4">
        <View className={`flex-1 rounded-2xl p-4 bg-muted/50 ${Platform.OS === 'ios' ? 'shadow-lg' : ''}`}>
          <View className="flex-row items-center mb-2">
            <Ionicons name="calendar-outline" size={20} color={iconColor} />
            <Text className="text-sm font-medium ml-2 text-muted-foreground">Tento mesiac</Text>
          </View>
     ...
     <View className={`flex-1 rounded-2xl p-4 bg-muted/50 ${Platform.OS === 'ios' ? 'shadow-lg' : ''}`}>
          <View className="flex-row items-center mb-2">
            ...
            <Text className="text-sm font-medium ml-2 text-muted-foreground">Celkovo</Text>
          </View>
          <Text className="text-2xl font-bold text-card-foreground">
