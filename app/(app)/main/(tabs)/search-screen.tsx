...
export default function SearchScreen() {
  ...
  const getProductsForSubcategory = (subcategoryId: number) => {
    const { data: { products: categoryProducts = [] } = {}, isLoading: areCategoryProductsLoading } = useGetProducts(
      {
    ...
    const outputCategoryProducts: ShopItemDto[] = categoryProducts?.map(({ products }) => products?.[0]).filter((product): product is ShopItemDto => Boolean(product)) || [];
  ...
  console.log('SearchScreen render - searchQuery:', searchQuery, 'displaySearchResult:', displaySearchResult, 'selectedCategory:', selectedCategory?.name);
